...
$("#devices a").on("click", function(){ // when they click on a device link we want to trigger the second tip.
  if (!hasClicked) {
    $(".joyride-tip-guide .joyride-next-tip").eq(2).click();
    hasClicked=true;
  }
});
...
$(".joyride-tip-guide").find(".joyride-next-tip").eq(1).remove(); // removes next button on the devices tip, we want them to click a button, NOT click next or else the rest of the stuff won't look right
